...
from middlewared.common.listen import ServiceListenDelegate
...
class InterfaceService(Service):
    ...
    async def services_restarted_on_sync(self):
        ...
        """
        return {pd.delegate.service: pd.addresses for pd in await self.listen_delegates_prepare()
                if isinstance(pd.delegate, ServiceListenDelegate)}
    ...
    async def listen_delegates_prepare(self):
        ...
        for delegate in self.delegates:
            state = await delegate.get_listen_state()
...
async def interface_post_sync(middleware):
    ...
    for pd in PRE_SYNC_LISTEN_DELEGATES:
        middleware.logger.info("Resetting listen IPs on delegate %r because %r addresses were removed", pd.delegate,
                               pd.addresses)
        asyncio.ensure_future(pd.delegate.reset_listens())
